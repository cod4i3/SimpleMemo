...
chrome.storage.local.get("hold_content", function (items) {
  ...
  if (items.hold_content === undefined) {
    (<HTMLInputElement>document.getElementById("contents")).value = "";
  } else {
    (<HTMLInputElement>document.getElementById("contents")).value =
      items.hold_content;
    countSet(contents);
...
document.getElementById("contents").onkeyup = function () {
  // hold_contentに入力値を保存
  let contents: string = (<HTMLInputElement>document.getElementById("contents"))
    .value;
...
// ここからは全て文字数やバイト数のカウントについて
function countSet(contents: string): void {
  CharCount(contents);
  ByteCount(contents);
  LineCount(contents);
}
...
// 文字数カウント
function CharCount(contents: string) {
  let cnt: string = contents.length.toString(10);
  document.getElementById("charCounter").textContent = cnt + "文字";
}

// バイト数カウント
function ByteCount(contents: string) {
  let cnt: number = 0;
  for (let i: number = 0; i < contents.length; i++) {
    let chr: number = contents.charCodeAt(i);
...
// 行数カウント
function LineCount(contents: string) {
  let cnt: string = contents.split(/\r|\r\n|\n/g).length.toString(10);
